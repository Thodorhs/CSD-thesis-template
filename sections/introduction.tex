...
DRAM memory and hybrid heaps \note{jk: term hybrid heap is when a managed heap
  is extended over DRAM and a slower memory tier. so hybrid heaps is not
correct}. Using an extended heap over a local storage device
(e.g TeraHeap) typically  leverages the physical proximity and direct connection
to enhance access speeds and reliability \note{jk: I do not understand the
previous sentence}. In contrast, remote storage device
implementations may benefit from centralized management and scalability but
often at the cost of increased latency\note{jk: also storage device add latency.
  I think you should discuss that some work leverage the capacity of the fast
  storage devices. Other work, see that interconnects such as RDMA become
faster, so they leverage remote memory}. Similarly, utilizing remote DRAM for
...
infrastructure \note{jk: how do you know if the infrastructure is robust. say
what you want direct, no extra words.} to minimize latency and ensure data
...
method. \note{jk: I like the last two sentences}
...
%  \item Possible approaches for more memory
 % \begin{itemize}
  %  \item local storage device (e.g., NVMe SSD) or remote storage device or remote memory
...
%    \item SPDK-nvme-of can reduce device latency
 % \end{itemize} 
  %\item We use TeraHeap which is the state of the art {Evaluation + some high level results}
%\end{itemize}
